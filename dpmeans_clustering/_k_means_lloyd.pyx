...
def specialized_cosine(
    ...
    with nogil, parallel(num_threads=n_threads):
        ...
        for chunk_idx in prange(n_chunks, schedule='static'):
            ...
            _specialized_cosine_dense(
                X[start: end],
                x_squared_norms[start: end],
                centers_old,
                centers_squared_norms,
                min_distances[start: end],
...
cdef void _specialized_cosine_dense(
        floating[:, ::1] X,                   # IN READ-ONLY
        floating[::1] x_squared_norms,        # IN
        floating[:, ::1] centers_old,         # IN
    ...
    """
    cdef:
        int n_samples = x_squared_norms.shape[0]
        int n_clusters = centers_old.shape[0]
    ...
    for i in range(n_samples):
        for j in range(n_clusters):
            pairwise_distances[i * n_clusters + j] = 0.0
    ...
    for i in range(n_samples):
        ...
        for j in range(0, n_clusters):
            sq_dist = (pairwise_distances[i * n_clusters+j]/(2*x_squared_norms[i:i+1][0]**(0.5)*centers_squared_norms[j]**(0.5)))   +1.0/2.0
            if sq_dist < min_sq_dist:
                min_sq_dist = sq_dist
                label = j
        min_distances[i] = min_sq_dist
...
cdef void _update_chunk_sparse(
    ...
    for i in range(n_samples):
        ...
        for j in range(n_clusters):
            ...
            sq_dist = (-1*sq_dist/2*(centers_squared_norms[j]*x_squared_norms[i:i+1][0])) +1.0/2.0
            if sq_dist < min_sq_dist:
                min_sq_dist = sq_dist
                label = j
...
cdef void _update_chunk_dense_with_min_sample(
    ...
    for i in range(n_samples):
        ...
        label = 0
        for j in range(0, n_clusters):
            sq_dist = (pairwise_distances[i * n_clusters+j]/(2*x_squared_norms[i:i+1][0]**(0.5)*centers_squared_norms[j]**(0.5)))   +0.5
            if sq_dist < min_sq_dist:
                min_sq_dist = sq_dist
                label = j
