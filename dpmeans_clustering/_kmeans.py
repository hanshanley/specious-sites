...
from ._k_means_lloyd import specialized_cosine
...
import pickle
FIRST_FOLDER = '/mnt/projects/qanon_proj/RussianTelegramChannels/new_weibo_mpnet_centers/'
###############################################################################
...
def _labels_inertia(X, sample_weight, x_squared_norms, centers, n_threads=1):
    ...
    print(labels)
    print('_labels_inertia end')
    min_distances = np.full(n_samples, -1, dtype=np.float32)
    specialized_cosine(X,
                         x_squared_norms,
                         centers,
                         min_distances,
                         n_threads)
...
class KMeans(TransformerMixin, ClusterMixin, BaseEstimator):
    ...
    def fit_predict(self, X, y=None, sample_weight=None):
        ...
        """
        actual_self = self.fit(X, sample_weight=sample_weight)
        return  actual_self.cluster_centers_
    
    
    def fit_predict_centers(self, X, y=None, sample_weight=None):
        """Compute cluster centers and predict cluster index for each sample.
        ...
        """
        actual_self = self.fit(X, sample_weight=sample_weight)
        return  actual_self.cluster_centers_,actual_self._counts
...
class MiniBatchKMeans(KMeans):
    ...
    def _mini_batch_convergence(
        ...
        print('MINI BATCH CONVERGENCE STEP')
...
class MiniBatchDPMeans(KMeans):
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        # Initialize counts
        # self._counts = np.array(self.counts, dtype=X.dtype)
        ...
        n_steps = (64 * n_samples) // self._batch_size +1
        #n_steps = max(n_steps,5000)
        max_steps =1000000
        #n_steps = (self.max_iter * n_samples) // max(self.day_indexes)
        iter_time = [0]
        self.iter_centers = [None]
        print('HERE')
        initial_add = n_samples // self._batch_size
        initial_add_repeats = np.repeat(np.arange(initial_add+1), self.n_introduce)
        print(initial_add_repeats)
        with threadpool_limits(limits=1, user_api="blas"):
            n_steps = max_steps #min(n_steps,max_steps)
            for i in range(n_steps):
                # Sample a minibatch from the full dataset
                #minibatch_indices = np.arange(i*self._batch_size,(i+1)*self._batch_size) % n_samples
                #minibatch_indices = minibatch_indices.tolist()
                minibatch_indices = random_state.randint(0, n_samples, self._batch_size)

                ## Get the current day's information
                #if i % len(day_indexes) == 0:
                #    minibatch_indices = range(0, self.day_indexes[i%len(day_indexes)] )
                #    print('HERE')
                #else:
                #    minibatch_indices = range(self.day_indexes[i%len(day_indexes)-1], self.day_indexes[i%len(day_indexes)] )
                tic = time()
                # Perform the actual update step on the minibatch data
                (
                    batch_inertia,
                ...
                print('UPDATED:' + str(i ))
                print(self.domain)
                min_distances = np.full(self._batch_size, -1, dtype=np.float32)
                specialized_cosine(X[minibatch_indices],
                                         x_squared_norms[minibatch_indices],
                                         centers,
                                         min_distances,
                                         self._n_threads)
                actual_intertia = sum(min_distances)+(self.delta)*len(centers)
                print(max(min_distances))
                print(np.average(min_distances))
                ...
                centers, centers_new = centers_new, centers
                if i % 100 ==0:
                    with open(FIRST_FOLDER+self.domain+'_2022_centers2.obj', 'wb') as outfile:
                        pickle.dump(centers, outfile)
                        outfile.close()
                    with open(FIRST_FOLDER+self.domain+'_2022_center_counts2.obj', 'wb') as outfile:
                ...
                if new_cluster is False and  self._mini_batch_convergence(
                    i, n_steps, n_samples, centers_squared_diff, actual_intertia
                ):
                    print('MINI BATCH CONVERGENCE')
                    break
